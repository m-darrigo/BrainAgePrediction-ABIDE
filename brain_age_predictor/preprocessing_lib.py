...
def alldistr_img(df: pd.DataFrame) -> None:
    """
...
def corr_distr(df_corr: pd.DataFrame) -> None:
    '''
...
def top_corr_relations(
        df: pd.DataFrame,
        y: pd.Series,
        target_variable: str = "AGE_AT_SCAN",
    ...
    Selects the top ``num_top_feature`` features based on correlation and generates
    regression plots for each feature against the target variable.
    Regression plots are arranged in a grid with 3 columns.
    ...
    y: pandas.Series
        Target variable to be correlated with features.
    target_variable: str
        Column name of the target variable. Default is "AGE_AT_SCAN".
    num_top_features: int
        Number of top features to consider. Default is 30.
...
def scaled_distributions(df: pd.DataFrame) -> None:
    """
    Visualizes scaled distributions of columns with high variance.

    This function takes a DataFrame and applies two scalers
    (:py:obj:`StandardScaler <sklearn.preprocessing.StandardScaler>` and
    :py:obj:`PowerTransformer <sklearn.preprocessing.PowerTransformer>`)
    to columns with high variance. Subsequently, it visualizes the scaled distributions using
    Seaborn's :py:func:`displot <seaborn.displot>`.
    ...
    Note
    ------
    This function identifies columns with high variance based on kurtosis, applies
    :py:obj:`StandardScaler <sklearn.preprocessing.StandardScaler>` and
    :py:obj:`PowerTransformer <sklearn.preprocessing.PowerTransformer>` to the selected columns,
    and visualizes the scaled distributions using Seaborn's
    :py:func:`displot <seaborn.displot>`.

    The top 20 columns with the highest kurtosis values are chosen for visualization. Two scalers,
    :py:obj:`StandardScaler <sklearn.preprocessing.StandardScaler>` and
    :py:obj:`PowerTransformer <sklearn.preprocessing.PowerTransformer>`,
    are applied to create two sets of scaled distributions.
...
def pca_variance(df: pd.DataFrame) -> None:
    """
    Visualizes the explained variance of Principal Components Analysis
    (:py:obj:`PCA <sklearn.decomposition.PCA>`) on scaled data.

    This function takes a DataFrame, applies two scalers
    (:py:obj:`StandardScaler <sklearn.preprocessing.StandardScaler>` and
    :py:obj:`PowerTransformer <sklearn.preprocessing.PowerTransformer>`),
    ...
    Note
    ------
    This function applies :py:obj:`StandardScaler <sklearn.preprocessing.StandardScaler>` and
    :py:obj:`PowerTransformer <sklearn.preprocessing.PowerTransformer>` to the DataFrame,
